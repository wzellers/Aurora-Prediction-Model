...
class Aurora(torch.nn.Module):
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        batch = dataclasses.replace(
            batch,
            static_vars={k: v.repeat(B, T, 1, 1) for k, v in batch.static_vars.items()},        )
        ...
        with torch.autocast(device_type="cuda") if self.autocast else contextlib.nullcontext():
            print(">>> Shape before Swin3D backbone:", x.shape)
            x = self.backbone(
                x,
                lead_time=self.timestep,
                patch_res=patch_res,
    ...
    def _pre_encoder_hook(self, batch: Batch) -> Batch:
        ...
        expected_h = batch.atmos_vars["t"].shape[-2]
        expected_w = batch.atmos_vars["t"].shape[-1]
        ...
        pad_h = expected_h - lat.shape[0]
        pad_w = expected_w - lon.shape[1]

        if pad_h > 0 or pad_w > 0:
            lat = F.pad(lat, (0, pad_w, 0, pad_h))
            lon = F.pad(lon, (0, pad_w, 0, pad_h))

        # ✅ Update metadata instead of returning dict
        batch.metadata.lat = lat
        batch.metadata.lon = lon
